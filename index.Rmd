...
For this article, I'm using the latest dev version of `rayshader` from GitHub (version `r packageVersion("rayshader")` at the time of writing), which is significantly faster than the version I had installed from CRAN. You can download this version with the code below. Many of the code-snippets in this article also use custom functions I wrote for this post. All these functions are available in the [git source repo](https://github.com/wcmbishop/rayshader-demo) for this article, under the `R` folder, along with a single file of all the code samples in `code-samples.R`.^[You can find the source code for this article, including all custom R functions (under the `R` folder), in the [git repo here](https://github.com/wcmbishop/rayshader-demo).] Now...let's get mapping.
...
The function below calculates an image size to use in the rest of our code, with the results shown in json. We can set the overall size of the image by defining the major dimension (e.g. `600` pixels, which sets the larger image dimension). Larger sizes will download more elevation data and make more detailed maps.
...
If you have suggestions or edits or thoughts on this material, please feel free to [create an issue](https://www.github.com/wcmbishop/rayshader-demo/issues/new) in the git repo or tweet at me (["@wcmbishop"](https://twitter.com/wcmbishop)).
